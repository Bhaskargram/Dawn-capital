...
export default function RefundPolicy() {
  ...
  useEffect(() => {
    axios.get(`${API}/config`)
      .then(res => setContent(res.data?.legals?.refundPolicy || 'Refund Policy not found.'))
      .catch(err => {
        console.error(err);
  ...
  return (
    <div style={{ background: '#0a0a0f', color: 'white', minHeight: '100vh', padding: '20px 20px 80px' }}>
      ...
      <div className="glass-card" style={{ maxWidth: '900px', margin: '0 auto', padding: '50px' }}>
        <h1 style={{ fontSize: '2.5rem', fontWeight: '800', marginBottom: '30px', color: '#C21B2F' }}>Refund Policy</h1>
